...
class Admincategory extends CI_Controller {
	...
	{				
		...
		$this->load->view('adminarea/category/category_board', $data);
	...
	{				
		...
		$data['title'] = "Food Category | Biriyani Hut"; 
		$data['meta'] = "Food Category Biriyani Hut";
		$data['keywords'] = "entree Lunch Dinner Special Deal Kids Meal Dessert Fresh Juice";	
		
		$this->load->view('adminarea/header', $data);
		$this->load->view('adminarea/category/category_form', $data);
		$this->load->view('adminarea/footer', $data);				
	...
	function edit($id)
	{				
		$row=$this->modal_db->getCategoryById($id);
		if($row)
		{
			$data['id'] = $row->id;
			$data['c_category']= $row->c_category;
			$data['c_description']= $row->c_description;
			$data['n_parent_id']= $row->n_parent_id;
	...
	function save()
	{
		$this->form_validation->set_rules('c_category', 'Category', 'trim|required|max_length[64]|xss_clean');
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$data['n_parent_id']= $this->input->post('n_parent_id');
		...
		else
		{
			...
			$c_category=$tdata['c_category']= $this->input->post('c_category');
			$tdata['c_description']= $this->input->post('c_description');
			$tdata['n_parent_id']= $this->input->post('n_parent_id');
			$tdata['c_slug']	= url_title(convert_accented_characters($c_category), 'dash', TRUE);
			...
			if ($this->modal_db->saveCategory($tdata,$id) == TRUE) 
			{
				$flash['type']='alert-success';
				$flash['title']='Success !';
				$flash['message']='Food Category Saved Successfully';
	...
	function delete($id)
	{				
		$row=$this->modal_db->deleteCategory($id);
		...
		else
		{
			$flash['type']='alert-error';
			$flash['title']='Failed !';
			$flash['message']='Category Could not be deleted!';
			$this->session->set_flashdata($flash);
			redirect('admincategory');	
		}			
